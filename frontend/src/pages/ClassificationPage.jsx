...
const MODELS = [
 { id: 'logistic_regression', label: 'Logistic Regression', tip: 'Fast and simple. Best for linearly separable data.' },
 { id: 'decision_tree',       label: 'Decision Tree',       tip: 'Easy to understand. Can overfit on small data.' },
 { id: 'random_forest',       label: 'Random Forest',       tip: 'Robust and accurate. Good default choice.' },
 { id: 'svm',                 label: 'SVM',                 tip: 'Great for high-dimensional data. Slower on large datasets.' },
 { id: 'knn',                 label: 'KNN',                 tip: 'Simple nearest-neighbour method. Slow on big data.' },
]
...
export default function ClassificationPage() {
 ...
 useEffect(() => {
   ...
   setDs(parsed)
   setCsvRaw(c || '')
   // Default: first categorical col as target, all numeric as features
 ...
 async function handleTrain() {
   ...
   if (!csvRaw)          { setError('CSV data missing. Please re-upload your file.'); return }
   setError('')
   setLoading(true)
   try {
     ...
     <main style={{ maxWidth: 860, margin: '0 auto', padding: '40px 24px' }}>
       {!results ? (
         <div>
           ...
           {/* Target column */}
           <Section label='2. Target column (what to predict)'>
           ...
           {/* Feature columns */}
           <Section label='3. Feature columns (inputs to the model)'>
           ...
           {/* Test split */}
           <Section label={`4. Test split — ${Math.round(testSize * 100)}% held back for testing`}>
...
function ClassificationResults({ results, onReset }) {
 const metrics = [
   { label: 'Accuracy',  value: results.accuracy  + '%', color: '#6c63ff' },
   { label: 'F1 Score',  value: results.f1        + '%', color: '#0ea5e9' },
   { label: 'Precision', value: results.precision + '%', color: '#10b981' },
   { label: 'Recall',    value: results.recall    + '%', color: '#f59e0b' },
 ]
 return (
   <div>
     ...
     <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 10, marginBottom: 24 }}>
       {metrics.map(m => (
         <div key={m.label} style={{ background: '#13131a', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 10, padding: '16px 14px', textAlign: 'center' }}>
           <div style={{ fontSize: 24, fontWeight: 600, color: m.color, fontFamily: 'monospace', marginBottom: 4 }}>{m.value}</div>
     ...
     <button onClick={() => sessionStorage.setItem('results', JSON.stringify(results))} style={{
       width: '100%', padding: '13px', background: '#13131a', border: '1px solid rgba(108,99,255,0.3)',
       borderRadius: 10, color: '#a09af0', fontSize: 14, fontWeight: 500, cursor: 'pointer',
     }}>
       Save results (Day 5 — code export coming)
     </button>
   </div>
 )
}
